...
# --- 1. DEFINISI STRUKTUR DATA (Model Final) ---
...
class KomponenMK:
    ...
    jenis: str
    jumlah_kelas: int
    # Fitur Baru: Jumlah sesi untuk praktikum
    jumlah_sesi_praktikum: int # Default 1, bisa 2 atau lebih
    semester: int
...
class JadwalItem:
    ...
    # ID unik untuk setiap item jadwal dalam satu run
    unique_id: int
    komponen_mk_id: int
    kelas: str
    # Fitur Baru: Sesi ke-berapa untuk praktikum
    sesi_ke: int # 1 atau 2
    pengajar_id: int
    waktu_id_start: int
    ruangan_id: int or None # Bisa None jika online
    # Fitur Baru: Status online
    daring: bool # True jika online
    # Data tambahan
    semester: int
    durasi_blok: int
...
def setup_database(db_name="jadwal_advanced.db"):
    """Membuat DB dengan skema baru yang mendukung sesi dan kelas online."""
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()

    tables = ['jadwal', 'pengampu', 'waktu', 'komponen_mk', 'mata_kuliah', 'pengajar', 'ruangan']
    ...
    # Data Sampel Baru
    ruangan_data = [(1, 'R-Teori-A', 'teori'), (2, 'Lab-Komputer-1', 'praktek')] # Hanya 1 ruang teori
    ...
    komponen_mk_data = [
        # Basis Data: Praktikum dipecah 2 sesi karena lab terbatas
        (1, 1, 'Teori Basis Data', 2, 'teori', 1, 1),
        (2, 1, 'Praktikum Basis Data', 1, 'praktek', 1, 2), # jumlah_sesi_praktikum = 2
        # Algoritma: Praktikum cukup 1 sesi
    ...
    for hari in ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat']:
        for jam in range(8, 17):
            for menit in [0, 45]:
                ...
                waktu_data.append((waktu_id, hari, f"{start_h:02d}:{start_m:02d}", f"{end_h:02d}:{end_m:02d}"))
                waktu_id += 1

    for table, data in [('ruangan', ruangan_data), ('pengajar', pengajar_data), ('mata_kuliah', mata_kuliah_data), ('komponen_mk', komponen_mk_data), ('waktu', waktu_data), ('pengampu', pengampu_data)]:
        placeholders = ','.join(['?'] * len(data[0]))
        cursor.executemany(f"INSERT INTO {table} VALUES ({placeholders})", data)

    conn.commit()
    conn.close()
    print(f"Database '{db_name}' dengan fitur sesi & online siap digunakan.")

def load_data_from_db(db_name="jadwal_advanced.db"):
    """Memuat semua data dari DB dan mempersiapkan struktur data pendukung."""
    global ruangans, pengajars, waktus, komponen_mks, ruangan_map, pengajar_map, waktu_map, komponen_mk_map, pengampu_map, waktu_start_options
    conn = sqlite3.connect(db_name)
...
def calculate_cost(schedule: List[JadwalItem]) -> int:
    """Menghitung biaya, kini dengan penalti untuk kelas online."""
    cost = 0
    PENALTI_BENTROK = 100
    PENALTI_DARING = 5 # Penalti kecil untuk kelas online
    ...
    for item in schedule:
        ...
        for blok_id in waktu_blok_ids:
            ...
            # Bentrok Mahasiswa
            if ((item.semester, item.kelas), blok_id) in mahasiswa_waktu_check: cost += PENALTI_BENTROK
            mahasiswa_waktu_check.add(((item.semester, item.kelas), blok_id))
...
def get_neighbor_solution(schedule: List[JadwalItem]) -> List[JadwalItem]:
    ...
    # Pilihan mutasi: ganti waktu, ganti ruang, atau ganti status online
    move_type = random.random()
    if move_type < 0.6: # 60% ganti waktu
        item_to_change.waktu_id_start = random.choice(waktu_start_options[item_to_change.durasi_blok])
    elif move_type < 0.8 and not item_to_change.daring: # 20% ganti ruang (jika tidak online)
    ...
    elif item_to_change.jenis == 'teori' and ruangan_teori_ids: # 20% ganti status online (hanya teori)
...
def simulated_annealing(max_iterations=300000):
    print("\nMemulai proses optimasi (dengan sesi & online)...")
    temp = 1000.0
...
def print_schedule(schedule: List[JadwalItem], cost: int):
    ...
    bentrok = cost // 100
    online = (cost % 100) // 5
    print(f"Status: {bentrok} bentrok ditemukan, {online} kelas dijadwalkan online.")
    print("-" * 45 + "\n")
    ...
    for item in schedule:
        ...
        print(f"[{w_start.jam_mulai}-{w_end.jam_selesai}] {komp.nama_komponen}{sesi_info} - Kelas {item.kelas}\n"
              f"  -> Pengajar: {pengajar.nama} ({pengajar.tipe})\n"
              f"  -> Lokasi  : {lokasi}\n")

# --- 5. EKSEKUSI PROGRAM ---
if __name__ == "__main__":
    DB_FILE = "jadwal_advanced.db"
