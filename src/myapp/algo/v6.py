...
def setup_database(db_name: str = "jadwal_advanced.db"):
    ...
    for table in tables:
        _ = cursor.execute(f"DROP TABLE IF EXISTS {table};")
    ...
    waktu_data: list[tuple[int, str, str, str]] = []
    waktu_id = 1
    ...
    _ = cursor.executemany("INSERT INTO ruangan VALUES (?,?,?)", ruangan_data)
    _ = cursor.executemany("INSERT INTO pengajar VALUES (?,?,?)", pengajar_data)
    _ = cursor.executemany("INSERT INTO mata_kuliah VALUES (?,?,?)", mata_kuliah_data)
    _ = cursor.executemany(
        "INSERT INTO komponen_mk VALUES (?,?,?,?,?,?,?)", komponen_mk_data
    )
    _ = cursor.executemany("INSERT INTO waktu VALUES (?,?,?,?)", waktu_data)
    _ = cursor.executemany("INSERT INTO pengampu VALUES (?,?,?)", pengampu_data)
    _ = cursor.executemany(
        "INSERT INTO preferensi_pengajar (pengajar_id, waktu_id) VALUES (?,?)",
        preferensi_data,
    )
...
def load_data_from_db(db_name: str = "jadwal_advanced.db"):
    ...
    global \
        ...
        pengampu_map, \
        waktu_start_options, \
        preferensi_hindari_set
    conn = sqlite3.connect(db_name)
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    ruangans = [
        Ruangan(**row) for row in cursor.execute("SELECT * FROM ruangan").fetchall()
    ]
    ruangan_map = {r.id: r for r in ruangans}
    pengajars = [
        Pengajar(**row) for row in cursor.execute("SELECT * FROM pengajar").fetchall()
    ]
    pengajar_map = {p.id: p for p in pengajars}
    waktus = [
        Waktu(**row)
        for row in cursor.execute("SELECT * FROM waktu ORDER BY id").fetchall()
    ]
    ...
    komponen_mks = [
        KomponenMK(**row) for row in cursor.execute(query_komponen).fetchall()
    ]
    komponen_mk_map = {k.id: k for k in komponen_mks}
    pengampu_map = {
        row["komponen_mk_id"]: row["pengajar_id"]
        for row in cursor.execute("SELECT * FROM pengampu").fetchall()
    }

    # Memuat data preferensi
    preferensi_hindari_set = {
        (row["pengajar_id"], row["waktu_id"])
        for row in cursor.execute("SELECT * FROM preferensi_pengajar").fetchall()
    }

    max_durasi = max(k.durasi_blok for k in komponen_mks) if komponen_mks else 1
    waktu_by_hari = {
        h: [w.id for w in sorted(waktus, key=lambda x: x.id) if w.hari == h]
        for h in set(w.hari for w in waktus)
    }
...
def save_schedule_to_db(
    schedule: list[JadwalItem], db_name: str = "jadwal_advanced.db"
):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    ...
    _ = cursor.executemany(
        "INSERT INTO jadwal (komponen_mk_id, kelas, sesi_ke, ruangan_id, pengajar_id, waktu_id_start, durasi_blok, daring) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        schedule_data,
    )
    conn.commit()
    conn.close()
...
def generate_initial_solution() -> list[JadwalItem]:
    ...
    initial_schedule: list[JadwalItem] = []
            ...
            for sesi in range(1, num_sessions + 1):
                ...
                if komp.jenis == "teori":
                    if random.random() < 0.1 and ruangan_teori_ids:
                        ...
                        ruangan_id = (
                            random.choice(ruangan_teori_ids)
                            if ruangan_teori_ids
                            else None
                        )
                        if not ruangan_id:
                            is_daring = True
                    ...
                    ruangan_id = (
                        random.choice(ruangan_praktek_ids)
                        if ruangan_praktek_ids
                        else None
                    )
                ...
                initial_schedule.append(
                    JadwalItem(
                        ...
                        komponen_mk_id=komp.id,
                        kelas=kelas_char,
                        sesi_ke=sesi,
                        pengajar_id=pengampu_map[komp.id],
                        waktu_id_start=start_time_id,
                        ruangan_id=ruangan_id,
                        daring=is_daring,
                        semester=komp.semester,
                        durasi_blok=komp.durasi_blok,
                        jenis=komp.jenis,
                    )
                )
                unique_id_counter += 1
...
def calculate_cost(schedule: list[JadwalItem]) -> int:
    ...
    ruangan_waktu_check: set[tuple[int, int]] = set()
    pengajar_waktu_check: set[tuple[int, int]] = set()
    # Logika baru untuk bentrok mahasiswa yang lebih akurat
    mahasiswa_waktu_check: dict[
        int, set[tuple[str, int, str, int] | tuple[str, int, str]]
    ] = {}
    ...
    for item in schedule:
        ...
        waktu_blok_ids = range(
            item.waktu_id_start, item.waktu_id_start + item.durasi_blok
        )

        # --- Logika Bentrok Mahasiswa Baru ---
        is_split = (
            item.jenis == "praktek"
            and komponen_mk_map[item.komponen_mk_id].jumlah_sesi_praktikum > 1
        )

        current_item_groups_to_add: set[
            tuple[str, int, str, int] | tuple[str, int, str]
        ] = set()
        if is_split:
            # Sesi praktikum yang dipecah hanya membuat grup sesinya sibuk
            current_item_groups_to_add.add(
                ("session", item.semester, item.kelas, item.sesi_ke)
            )
        else:
        ...
        for blok_id in waktu_blok_ids:
            ...
            if not item.daring and item.ruangan_id is not None:
                if (item.ruangan_id, blok_id) in ruangan_waktu_check:
                    cost += PENALTI_BENTROK

                ruangan_waktu_check.add((item.ruangan_id, blok_id))
            if (item.pengajar_id, blok_id) in pengajar_waktu_check:
                cost += PENALTI_BENTROK
            ...
            busy_groups_in_block: set[
                tuple[str, int, str, int] | tuple[str, int, str]
            ] = mahasiswa_waktu_check[blok_id]
            is_clash = False
            ...
            if not is_split:
                for group in busy_groups_in_block:
                    if (
                        group[0] == "session"
                        and group[1] == item.semester
                        and group[2] == item.kelas
                    ):
                        is_clash = True
                        break
            ...
            if (
                is_split
                and ("session", item.semester, item.kelas, item.sesi_ke)
                in busy_groups_in_block
            ):
                is_clash = True
...
def get_neighbor_solution(schedule: list[JadwalItem]) -> list[JadwalItem]:
    ...
    neighbor: list[JadwalItem] = copy.deepcopy(schedule)
    if not neighbor:
        return neighbor
    ...
    if move_type < 0.6:
        item_to_change.waktu_id_start = random.choice(
            waktu_start_options[item_to_change.durasi_blok]
        )
...
def simulated_annealing(max_iterations: int = 300000):
    ...
    for i in range(max_iterations):
        ...
        if current_cost < best_cost:
            ...
            if best_cost == 0:
                print(
                    "Solusi optimal (tanpa bentrok & semua preferensi terpenuhi) ditemukan!"
                )
                break
        temp *= cooling_rate
...
def print_schedule(schedule: list[JadwalItem], cost: int):
    ...
    print(
        f"Status: {bentrok} bentrok, {online} kelas online, {preferensi} preferensi terlanggar."
    )
    print("-" * 45 + "\n")
    ...
    for item in schedule:
        ...
        print(
            f"[{w_start.jam_mulai}-{w_end.jam_selesai}] {komp.nama_komponen}{sesi_info} - Kelas {item.kelas} (Sem {item.semester})\n"
