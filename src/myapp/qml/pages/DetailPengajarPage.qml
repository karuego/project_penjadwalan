...
Page {
    ...
    Component.onCompleted: {
        if (action == "view") {
            ...
            tambahWaktu.visible = false;
            btnFinale.text = qsTr("Kembali");
        } else if (action == "edit")
            root.title = "Edit Pengajar";
        else
            return;

        // const data = contextBridgeRef.getPengajarFromProxyIndex(pengajarId);
        const data = contextBridgeRef.pengajarModel.getById(pengajarId);

        if (data.nama)
            textFieldNama.text = data.nama;

        if (data.tipe) {
            if (data.tipe === "dosen")
                radioDosen.checked = true;
            else if (data.tipe === "asdos")
                radioAsdos.checked = true;
            // tipeField.currentIndex = tipeField.model.indexOf(data.tipe);
        }

        // if (data.waktu) waktuField.text = data.waktu;
    }
}
